...
#define ADXL345_POWER_CTL 0x2D    ///< bit for enabling measurement mode
#define ADXL345_DATA_FORMAT 0x31  ///< bit for data format (resolution)
#define ADXL345_SETTING 0x08      ///< bit to start measurement mode and set data format to full resolution
#define ADXL345_XDATA 0x32        ///< bit to start reading the x-Axis data
#define ADXL345_YDATA 0x34        ///< bit to start reading the y-Axis data
#define ADXL345_ZDATA 0x36        ///< bit to start reading the z-Axis data
 ...
 */
typedef enum {MPXV, MPRLS, NO_PRESSURE, ADXL345} pressure_type_t;
...
void initSensors()
{
  ...
  //we found the ADXL345 sensor, start the initialization and override the Usage of the MPRLS pressure sensor
  if (result == 0) {
  
	sensor_pressure = ADXL345;

    Wire1.begin();

    // Enable measurement mode
    Wire1.beginTransmission(ADXL345_ADDR);
    Wire1.write(ADXL345_POWER_CTL);
    Wire1.write(ADXL345_SETTING);
    Wire1.endTransmission(true);

    // Set data format to full resolution -> 10bit
    Wire1.beginTransmission(ADXL345_ADDR);
    Wire1.write(ADXL345_DATA_FORMAT);
    Wire1.write(ADXL345_SETTING);
    Wire1.endTransmission(true);
  }
...
/**
   @name getADXL345Value
   @brief called periodically in order to read out ADXL345 acceleration data
   @param AXIS_ADDR: address of the register for the axis
   @return int axis value
*/
int getADXL345Value(byte AXIS_ADDR) {
  Wire1.beginTransmission(ADXL345_ADDR);
  Wire1.write(AXIS_ADDR);
  Wire1.endTransmission(false);
  Wire1.requestFrom(ADXL345_ADDR, 2, true);

  // Combine the two bytes into a 10-bit signed integer
  int axisValue = Wire1.read() | (Wire1.read() << 8);
...
void readPressure(struct I2CSensorValues *data)
{
  ...
  switch (sensor_pressure)
  {
    ...
    case ADXL345:
    {
      // get new value from ADXL345 chip
      float ADXL345x = getADXL345Value(ADXL345_XDATA); // Read X-axis
      float ADXL345y = getADXL345Value(ADXL345_YDATA); // Read Y-axis
      float ADXL345z = getADXL345Value(ADXL345_ZDATA); // Read Z-axis

      float ADXL345Vector = sqrt(ADXL345x*ADXL345x+ADXL345y*ADXL345y+ADXL345z*ADXL345z);  // get resulting vector from x,y,z

      Serial.print("ADXL345: ");
      Serial.println(ADXL345Vector);
    }
    break;
    case NO_PRESSURE:
    default:
